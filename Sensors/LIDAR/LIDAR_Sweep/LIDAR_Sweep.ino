...
void loop(){ 
  ...
  int input_1 = LIDARanalysis();        Serial.print("input 1: "); 
  int input_2 = LIDARbinAnalysis();
...
int LIDARanalysis(){
                      ...
                      // Display the calculated values
                       ...
                       if      (LIDARmin > 200)                    {return 0;}
                       else if (LIDARangle > 40 && LIDARmin <50)   {return 0;}
                       else if (LIDARangle < -40 && LIDARmin > 50) {return 0;}
                       else                                        {return 1;}
                      }
...
*/
int LIDARbinAnalysis(){
                         int BinMax = 0;        // Set LIDAR Max and min values to quatantied that will be exceeded by actual reading
                         int BinMin = 10000;
                         int Max = 0;             // Bin number of Max LIDAR averaging bin
                         int Min = 0;             // Bin number of Min LIDAR averaging bin
                         ...
                         for(int bin = 0; bin < binNumber; bin++) { if(BinMin > LIDARbins[bin]) {BinMin = LIDARbins[bin]; Min = bin;}
                                                                    if(BinMax < LIDARbins[bin]) {BinMax = LIDARbins[bin]; Max = bin;}
                                                                   }
 
                         // Assign anblges to the bin places    + Angles are left of center, - Angles are right of center
                         MinBinAngle = LIDARAngles[Min+1];
                         MaxBinAngle = LIDARAngles[Max+1];
 
                         // Dispaly values
                         Serial.print("Minimum bin value: "); Serial.print(BinMin); Serial.print("\t Bin#: "); Serial.print (Min); Serial.print("\t Angle: "); Serial.println(MinBinAngle);
                         Serial.print("Maximum bin value: "); Serial.print(BinMax); Serial.print("\t Bin#: "); Serial.print (Max); Serial.print("\t Angle: "); Serial.println(MaxBinAngle);
                         Serial.println(""); // Blank Line
                         
                         if (BinMin > 100) {return 0;}
                         else              {return 1;}
                        }

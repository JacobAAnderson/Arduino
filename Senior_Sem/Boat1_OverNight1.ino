...
void loop() {
  ...
  static bool stationkeeping;
  if (DayTime && Battery == HIGH ) {  // If its day time and the batteries are charged, do waypoint Nave
          ...
          gpsindex = sizeof(desLat)/sizeof(float)-1;

          static bool Return;
          if( distance > PARKING_RADIUS ) Return = true;     // If the boat drifts beyond the parking radius, tell it to come back to the parking waypoint
          if( distance < WAYPOINT_RADIUS ) Return = false;   // It the boat achived the waypoint radius, tell to drift

          if ( Return && !Thrusters ) ThrustersOn();
          else if ( Return && Thrusters ) Drive();
...
void BatteryCheck(void) { // Check the battery voltage to see if it is at an acceptable level ---> Called by UpdataSensors()
  ...
  else if((motor_PORT.voltage() < 12) || (motor_STAR.voltage() < 12)) Battery = LOW;                          // Batteries are low ---> Switch state to low
  else if(Battery == LOW && ((motor_PORT.voltage() > 13) || (motor_STAR.voltage() > 13 ))) Battery = HIGH;        // Batteries are full --> Switch state to high
  else;
...
void UpdateSensors(void) { // Reads the sensors, takes an average of 10 readings from analog sensors, writes the values to the SD card and transmits them to the electron vis I2C ---> Called in main loop
  ...
  const float pH_slope = -3.838;
  const float pH_int = 13.72;
...
*/
  ...
  if( GPS.fix ) { dataString += String(GPS.latitudeDegrees,6);
                  dataString += ",";
  ...
  if(Thrusters){  motor_PORT.update();
                  motor_STAR.update();
